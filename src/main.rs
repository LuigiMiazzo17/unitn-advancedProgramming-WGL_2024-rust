...
async fn add_edge(State(state): State<AppState>, Json(ids): Json<Edge>) -> impl IntoResponse {
    ...
    let from_exists = controller.drones.contains_key(&ids.0) || controller.servers.contains_key(&ids.1);
    let to_exists = controller.drones.contains_key(&ids.0) || controller.servers.contains_key(&ids.1);
